  ...
  const {
    ...
    const elements = useElements();
     ...
     useEffect(() => {
      ...
      fetch("https://intense-dawn-27289.herokuapp.com/create-payment-intent", {
        method: "POST",
...
<h3 className='mb-3 d-inline-block border-bottom border-2 border-danger '>Details</h3>
<input
      placeholder="Name"
...
<br/>
    ...
    <input className="my-3 py-1 w-100 rounded-pill border-warning ps-3" 
     placeholder="Address" 
     {...register("address")} 
     required
     />
...
<br/>
  ...
  <div className=' p-5 shadow-lg m-0'>
      ...
      />
      {processing ? <CircularProgress></CircularProgress>: <Button style={{backgroundColor:'goldenrod'}} className='py-1 my-3 cart-button px-4 border-0' type="submit" disabled={!stripe || success}>
        Pay {grandtotal.toFixed(2)}
      </Button>}
    

{
    error&&<Alert severity="error" sx={{mt:3,width:'100%',mx:'auto'}}>{error}</Alert>
}
{
    success&&<Alert severity="success" sx={{mt:3,width:'100%',mx:'auto'}}>{success}</Alert>
}
    </div>
    </Grid>
